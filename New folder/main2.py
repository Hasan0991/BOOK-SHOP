...
def add_product():
    ...
    for products in existing_products:
        ...
        elif not is_valid_number(price1):
...
def add_book():
    ...
    for books in existing_books:
        if books.get_name() == name:
            ...
            return
        elif not is_valid_number(price):
            showerror('GUI Python', 'Please enter a valid number')
            return
        else:
...
def options():
    ...
    if option == 1:
        ...
        btn_back8 = Button(frame_regis, text="Back <==", width=10, font='Calibri 12', bd=7, height=2, bg="red", fg='white',command=back_to_admin_from_regis,state=DISABLED)
        btn_back8.place(relx=0.13, rely=0.98, anchor='s')
        btn_back8['state'] = ACTIVE
...
def confirm_book_price():
    ...
    with open("book_data.json", "r") as f:
        ...
        for user in data:
            ...
            elif not is_valid_number(add_new_price1):
                showerror('GUI Python', 'Please enter a valid number')
                return
...
def search(event=None):
    ...
    if simvols.islower():
        new_simvols = simvols.title()
    else:
        new_simvols = simvols

    for item in data:
        if new_simvols in item['snack']:
            result = item['snack']
    ...
    for item1 in data1:
        if new_simvols in item1['name']:
            result = item1['name']
            showinfo('GUI Python', f'Book: {result} is in list')
            found = True
            break
...
def back_to_admin_from_book_add():
    frame_add_book.place_forget()
    root.geometry('650x600')
    frame_admin.place(relx=0, rely=0)


def back_to_admin_from_snack():
    frame_add_snack.place_forget()
    root.geometry('650x600')
    frame_admin.place(relx=0, rely=0)
...
rad_but2 = Radiobutton(frame_option, text='Add User', width=10, font='Arial 12', variable=var1, value=1,fg='blue' )
rad_but3 = Radiobutton(frame_option, text='Add Book', width=10, variable=var1, font='Arial 12', value=2, fg='blue')
rad_but4 = Radiobutton(frame_option, text='Add Snack', width=10, variable=var1, value=3, font='Arial 12',fg='blue' )
btn_option = Button(frame_option, text='Select', command=options,fg='blue')
btn_back5 = Button(frame_option, text='Back <==', font='Abadi 12', command=back_to_admin_from_option,fg='blue')
...
btn_back7 = Button(frame_add_book, text='Back <==', font='Calibri 15', command=back_to_admin_from_book_add)

btn_back7.place(relx=0.64, rely=0.8)
...
btn_back6 = Button(frame_add_snack, text='Back <==', font='Calibri 15', command=back_to_admin_from_snack)

btn_back6.place(relx=0.64, rely=0.8)
